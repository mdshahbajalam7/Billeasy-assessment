...
employee.post("/create", (req, res) => {
  ...
  database.query("SELECT COUNT(employees.id) FROM employees",(error, results) => {
      if (error) {
        throw error;
      }
    ...
    database.query(`SELECT * FROM department WHERE name = '${department}'`,(error, results) => {
      if (error) {
        throw error;
      }
  ...
  database.query('INSERT INTO employees (name, email,gender, mobile,department_id, created_at, updated_at) VALUES ($1, $2, $3, $4, $5,$6, $7) RETURNING *',
